...
def process_one_shard(corpus_params, params):
    corpus_type, fields, src_reader, edge_index_reader, edge_type_reader, node_type_reader, tgt_reader, align_reader, opt, \
    existing_fields, src_vocab, tgt_vocab = corpus_params
    i, (src_shard, edge_index_shard, edge_type_shard, node_type_shard, tgt_shard, align_shard, maybe_id,
        filter_pred) = params
    ...
    # For graph
    edge_index_data = {"reader": edge_index_reader, "data": edge_index_shard, "dir": None}
    edge_type_data = {"reader": edge_type_reader, "data": edge_type_shard, "dir": None}
    node_type_data = {"reader": node_type_reader, "data": node_type_shard, "dir": None}
    ...
    _readers, _data, _dir = inputters.Dataset.config(
        [('src', src_data), ('edge_index', edge_index_data), ("edge_type", edge_type_data),
         ("node_type", node_type_data), ('tgt', tgt_data), ('align', align_data)])
...
def build_save_dataset(corpus_type, fields, src_reader, edge_index_reader, edge_type_reader, node_type_reader,
    ...
    if corpus_type == 'train':
        ...
        srcs = opt.train_src
        edge_indexs = opt.train_edge_index  # for graph
        edge_types = opt.train_edge_type
        node_types = opt.train_node_type
        tgts = opt.train_tgt
        ids = opt.train_ids
        ...
        counters = None
        srcs = [opt.valid_src]
        edge_indexs = [opt.valid_edge_index]  # for graph
        edge_types = [opt.valid_edge_type]
        node_types = [opt.valid_node_type]
        tgts = [opt.valid_tgt]
        ids = [None]
    ...
    def shard_iterator(srcs, edge_indexs, edge_types, node_types, tgts, ids, aligns, existing_shards,
                       existing_fields, corpus_type, opt):
        """
        Builds a single iterator yielding every shard of every corpus.
        """
        for src, edge_index, edge_type, node_type, tgt, maybe_id, maybe_align in zip(srcs, edge_indexs, edge_types,
                                                                                     node_types, tgts, ids,
                                                                                     aligns):
            if maybe_id in existing_shards:  # 空
                if opt.overwrite:
            ...
            # for graph
            edge_index_shards = split_corpus(edge_index, opt.shard_size)
            edge_type_shards = split_corpus(edge_type, opt.shard_size)
            node_type_shards = split_corpus(node_type, opt.shard_size)
            tgt_shards = split_corpus(tgt, opt.shard_size)
            align_shards = split_corpus(maybe_align, opt.shard_size)
            for i, (ss, eis, ets, nts, ts, a_s) in enumerate(
                    zip(src_shards, edge_index_shards, edge_type_shards, node_type_shards, tgt_shards, align_shards)):
                yield (i, (ss, eis, ets, nts, ts, a_s, maybe_id, filter_pred))

    shard_iter = shard_iterator(srcs, edge_indexs, edge_types, node_types, tgts, ids, aligns, existing_shards,
                                existing_fields, corpus_type, opt)

    with Pool(opt.num_threads) as p:
        dataset_params = (
            corpus_type, fields, src_reader, edge_index_reader, edge_type_reader, node_type_reader, tgt_reader,
            align_reader, opt, existing_fields, src_vocab, tgt_vocab)
...
def preprocess(opt):
    ...
    build_save_dataset(
        'train', fields, src_reader, edge_index_reader, edge_type_reader, node_type_reader, tgt_reader, align_reader,
        opt)

    if opt.valid_src and opt.valid_tgt:
        logger.info("Building & saving validation data...")
        build_save_dataset(
            'valid', fields, src_reader, edge_index_reader, edge_type_reader, node_type_reader, tgt_reader,
            align_reader, opt)
