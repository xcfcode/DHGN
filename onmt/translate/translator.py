...
from onmt.utils.misc import tile, set_random_seed, report_matrix, sequence_mask
...
class Translator(object):
    ...
    def __init__(
        ...
        self.edge_index_reader = edge_index_reader
        self.edge_type_reader = edge_type_reader
        self.node_type_reader = node_type_reader
    ...
    def from_opt(
        ...
        edge_index_reader = inputters.str2reader["text"].from_opt(opt)  # For graph
        edge_type_reader = inputters.str2reader["text"].from_opt(opt)  # For graph
        node_type_reader = inputters.str2reader["text"].from_opt(opt)  # For graph
    ...
    def translate(
        ...
        # For graph
        edge_index_data = {"reader": self.edge_index_reader, "data": edge_index, "dir": None}
        edge_type_data = {"reader": self.edge_type_reader, "data": edge_type, "dir": None}
        node_type_data = {"reader": self.node_type_reader, "data": node_type, "dir": None}
        tgt_data = {"reader": self.tgt_reader, "data": tgt, "dir": None}
        _readers, _data, _dir = inputters.Dataset.config(
            [('src', src_data), ('edge_index', edge_index_data), ("edge_type", edge_type_data),
             ("node_type", node_type_data), ('tgt', tgt_data)])
    ...
    def _run_encoder(self, batch):
        ...
        """
        get hierarchical src data
        """
        src = batch.src["data"].unsqueeze(-1)
        src_lengths = batch.src["len"]  # node_batch
        recover = batch.src["recover"]  # node_batch
        node_nums = batch.src["batch_node_nums"]
        max_node_num = batch.src["max_node_num"]
        max_node_len = batch.src["max_node_len"]

        """
        create dynamic batch graph
        """
        # edge index
        _batch_edge_index = []
        for index, edge_index in enumerate(batch.edge_index):
            _batch_edge_index.append(
                (edge_index + max_node_num * index).to(src.device))  # add max_node_num at all index
        batch_edge_index = torch.cat(_batch_edge_index, dim=1).to(src.device)  # [2,total_edges]

        # edge_type
        batch_edge_type = torch.cat(batch.edge_type, dim=1).squeeze(0).to(src.device)  # [total_edges]

        # node type
        _batch_node_type = []
        for one in batch.node_type:
            if one.shape[1] < max_node_num:
                one_padded = torch.cat((one, torch.zeros(1, (max_node_num - one.shape[1]), dtype=torch.long)),
                                       dim=1).to(src.device)
                _batch_node_type.append(one_padded)
            else:
                _batch_node_type.append(one.to(src.device))

        batch_node_type = torch.cat(_batch_node_type, dim=1).squeeze(0).to(
            src.device)  # [node_batch:batch * max_node_num]

        batch_node_position = torch.zeros_like(batch_node_type)
        count = 3
        for index, node_type in enumerate(batch_node_type):
            if node_type == 0:  # pad
                batch_node_position[index] = 0
            elif node_type == 1:  # speaker
                batch_node_position[index] = 1
            elif node_type == 2:  # knowledge
                batch_node_position[index] = 2
            elif node_type == 3:  # utterance
                batch_node_position[index] = count
                if count != 35:
                    count = count + 1
                else:
                    count = 35
            if node_type == 0 or node_type == 2:
                count = 3
        batch_node_position.to(src.device)

        # tgt_outer = batch.tgt

        """hierarchical encoder：node encoder + graph encoder"""
        _, memory_bank, lengths = self.model.encoder(src, src_lengths, recover, node_nums, max_node_num, max_node_len,
                                                     batch_edge_index, batch_edge_type,
                                                     batch_node_type, batch_node_position)
        ...
        lengths = torch.tensor(orig_lengths, dtype=torch.int).to(src.device)
        ...
        enc_states = (encoder_hidden, encoder_hidden)
    ...
    def _translate_batch_with_strategy(
        ...
        results["batch"].src = (src, src_lengths)
