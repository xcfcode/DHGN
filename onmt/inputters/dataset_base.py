...
def process_edge_index(edge_index_str):
    """
    process edge index string into coo format which is used for torch geometric
    :param edge_index_str: format '0 2 , 0 3 , 2 1 , 3 1'
    :return:
    """
    res = []
...
def str2tensor(input):
    input = input.strip()
    res = [int(one.strip()) for one in input.split()]
    out = torch.tensor(res, dtype=torch.long)
    out = out.unsqueeze(0)
    return out
...
class Dataset(TorchtextDataset):
    ...
    def __init__(self, fields, readers, data, dirs, sort_key,
        ...
        for ex_dict in starmap(_join_dicts, zip(*read_iters)):
            ...
            """process graph related items"""
            ex_dict["edge_index"] = process_edge_index(ex_dict["edge_index"])  # process edge index
            ex_dict["edge_type"] = str2tensor(ex_dict["edge_type"])  # process edge_type
            ex_dict["node_type"] = str2tensor(ex_dict["node_type"])  # process node_type
