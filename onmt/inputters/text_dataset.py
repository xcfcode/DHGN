...
import six
import torch
from onmt.inputters.my_field import MySrcField
...
def text_sort_key_node_num(ex):
    return len(list(locate(ex.src[0], lambda a: a == "[SEP]")))
...
class TextMultiField(RawField):
    ...
    def process(self, batch, device=None):
        ...
        if self.fields[0][0] != "src":
            ...
            # for MySrcField
            res_dict = self.base_field.process(batch_by_feat[0], device=device)
            return res_dict
...
def text_fields(**kwargs):
    ...
    for i in range(n_feats + 1):
        ...
        if base_name == "src":
            sent_split = kwargs.get("sent_split", "[SEP]]")
            feat = MySrcField(
                init_token=bos, eos_token=eos,
                pad_token=pad, tokenize=tokenize,
                include_lengths=use_len, sent_split=sent_split)
        elif base_name == "tgt":
            feat = Field(
                init_token=bos, eos_token=eos,
                pad_token=pad, tokenize=tokenize,
