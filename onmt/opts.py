...
def model_opts(parser):
    ...
    """load params"""
    group.add('--load_type', '-load_type', default='full', choices=['full', 'embedding', 'encoder'], type=str,
              help="load params.")

    """##### graph related opts #####"""
    group.add('--encoder_type', '-encoder_type', type=str, default='hier', help="hierarchical encoder")

    group.add('--node_encoder', '-node_encoder', type=str, default='transformer',
              choices=['rnn', 'brnn', 'mean', 'transformer', 'cnn'],
              help="node encoder types")

    group.add('--graph_encoder', '-graph_encoder', type=str, default='hgt',
              choices=['gcn', 'rgcn', 'gat', 'hgt'],
              help="graph encoder types")

    group.add('--graph_in_dim', '-graph_in_dim', type=int, default=512,
              help="graph encoder input dim")

    group.add('--num_node_types', '-num_node_types', type=int, default=4,
              help="number of node types")

    group.add('--num_edge_types', '-num_edge_types', type=int, default=56,
              help="number of edge types")

    group.add('--graph_heads', '-graph_heads', type=int, default=8,
              help='Number of heads for graph attention')

    group.add('--graph_layers', '-graph_layers', type=int, default=2,
              help='Number of graph layers')

    group.add('--graph_dropout', '-graph_dropout', type=float, default=0.3,
              help='graph dropout')

    """###########"""
    group.add('--selective_gate', '-selective_gate',
              action='store_true',
              help="selective gate for knowledge representation")
    """###########"""
...
def preprocess_opts(parser):
    ...
    # for graph
    group.add('--train_edge_index', '-train_edge_index', required=True, nargs='+',
              help="Path(s) to the training edge index files")
    group.add('--train_edge_type', '-train_edge_type', required=True, nargs='+',
              help="Path(s) to the training edge type files")
    group.add('--train_node_type', '-train_node_type', required=True, nargs='+',
              help="Path(s) to the training node type files")
    ...
    # for graph
    group.add('--valid_edge_index', '-valid_edge_index',
              help="Path to the validation edge index file")
    group.add('--valid_edge_type', '-valid_edge_type',
              help="Path to the validation edge type file")
    group.add('--valid_node_type', '-valid_node_type',
              help="Path to the validation node type file")
...
def train_opts(parser):
    ...
    group.add('--valid_batch_size', '-valid_batch_size', type=int, default=16,
...
def translate_opts(parser):
    ...
    group.add('--src', '-src', required=True, default="data/test.src.txt",
              help="Source sequence to decode (one line per "
                   "sequence)")

    # for graph
    group.add('--edge_index', '-edge_index', required=True, default="data/test.edge_index.txt",
              help="Path to the edge index files")
    group.add('--edge_type', '-edge_type', required=True, default="data/test.edge_type.txt",
              help="Path(s) to the edge type files")
    group.add('--node_type', '-node_type', required=True, default="data/test.node_type.txt",
              help="Path to the node type files")
    ...
    group.add('--dynamic_dict', '-dynamic_dict', action='store_true', default=True,
              help="Create dynamic dictionaries")
    ...
    group.add('--beam_size', '-beam_size', type=int, default=5,
              help='Beam size')
    group.add('--min_length', '-min_length', type=int, default=15,
              help='Minimum prediction length')
    group.add('--max_length', '-max_length', type=int, default=1000,
